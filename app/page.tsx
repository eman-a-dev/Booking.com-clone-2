...
export default function Home() {
  return (
   <main className="bg-[#013B94]">
      ...
      <section className="mx-auto max-w-5xl mt-10 p-6 bg-white rounded-t-lg">
        <div className="pt-5">
          <h3 className="text-xl font-bold">Trending Destinations</h3>
          <p className="font-light">
            Most popular choices for travellers from around the
          </p>
        </div>

        <div className="flex space-x-4 py-5 overflow-x-scroll">
         {trending_data.map(item =>(
          <div key={item.id} className="space-y-l shrink-0 cursor-pointer">
            <img
            key={item.id}
